...
async function testBeadsIntegration() {
  ...
  if (hooks && hooks['agent.execute.before']) {
    ...
    if (beforeOutput.systemPrompt) {
      ...
      console.log(`   - System prompt length: ${beforeOutput.systemPrompt.length} characters`);
    ...
    if (beforeOutput.beadsTriage) {
      ...
      if (triage.recommendations && triage.recommendations.length > 0) {
        console.log(`   - Recommendations: ${triage.recommendations.length} items`);
  ...
  if (beforeOutput.systemPrompt) {
    ...
    console.log(`     - Contains Project Health: ${hasProjectHealth ? '✓' : '✗'}`);
    console.log(`     - Contains Quick Wins: ${hasQuickWins ? '✓' : '✗'}`);
    console.log(`     - Contains Blockers to Clear: ${hasBlockers ? '✓' : '✗'}`);
  }
  ...
  try {
    ...
    try {
      const { stdout } = await execAsync('bd ready', { maxBuffer: 1024 * 1024, timeout: 5000 });
      console.log(`   ✓ bd ready works: ${stdout.split('\n').length} tasks found`);
    } catch (error) {
      console.log(`   ⚠ bd ready failed (may not be installed): ${error.message}`);
    }

    // Test bv --robot-triage
    console.log('   Testing: bv --robot-triage');
    try {
      const { stdout } = await execAsync('bv --robot-triage', { maxBuffer: 5 * 1024 * 1024, timeout: 10000 });
      const triage = JSON.parse(stdout);
      console.log(`   ✓ bv --robot-triage works:`, triage.success ? 'success' : 'failed');
      if (triage.success && triage.triage) {
        console.log(`     - Data hash: ${triage.data_hash?.substring(0, 8)}...`);
        console.log(`     - Triage keys:`, Object.keys(triage.triage));
        if (triage.triage.recommendations && triage.triage.recommendations[0]) {
          console.log('     First recommendation keys:', Object.keys(triage.triage.recommendations[0]));
          console.log('     Has unblocks key?', 'unblocks' in triage.triage.recommendations[0]);
        }
      }
    } catch (error) {
      console.log(`   ⚠ bv --robot-triage failed (may not be installed): ${error.message}`);
    }

    // Test bv --robot-insights
    console.log('   Testing: bv --robot-insights');
    try {
      const { stdout } = await execAsync('bv --robot-insights', { maxBuffer: 5 * 1024 * 1024, timeout: 10000 });
      const insights = JSON.parse(stdout);
      console.log(`   ✓ bv --robot-insights works:`, insights.success ? 'success' : 'failed');
      if (insights.success && insights.triage) {
        console.log(`     - Metrics computed: ${Object.keys(insights.triage).length}`);
      }
    } catch (error) {
      console.log(`   ⚠ bv --robot-insights failed (may not be installed): ${error.message}`);
    }
  ...
  try {
    const beadsConfig = JSON.parse(await readFile('./beads_config.json', 'utf-8'));
  ...
  try {
    const opencodeConfig = JSON.parse(await readFile('./opencode.json', 'utf-8'));
    const hasBeadsPlugin = opencodeConfig.plugin && opencodeConfig.plugin.includes('./plugin/beads.mjs');
    console.log(`   ✓ opencode.json ${hasBeadsPlugin ? 'includes' : 'does not include'} beads plugin`);
  ...
  console.log('- All sections included (Recommendations, Quick Wins, Blockers) ✓');
