...
- When closing tasks via `bd update --status closed`, always provide `--session` parameter
- Session ID satisfies Beads CHECK constraint: `(status = 'closed' AND closed_at IS NOT NULL)`
- Generate session ID with: `--session "ses_$(date +%s)"` or use `CLAUDE_SESSION_ID` env var
- Example: `bd update opencode-xxx --session "ses_$(date +%s)" --status closed --notes "Completed: reason"`
- Without `--session`, Beads cannot properly close tasks (database CHECK constraint violation)
