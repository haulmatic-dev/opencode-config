...
async function testBeadsClient() {
  ...
  try {
    ...
    results.push({ function: 'ready() / parseOutput()', status: 'success', taskCount: parsed.length });
  } catch (error) {
    console.log('   ✗ ready() failed:', error.message);
    results.push({ function: 'ready() / parseOutput()', status: 'failed', error: error.message });
  }

  // Test 2: show(id)
  console.log('\n2. Testing show(id)...');
  try {
    const { stdout } = await execAsync('bd show opencode-k4w', { maxBuffer: 1024 * 1024 });
    ...
    results.push({ function: 'show(id)', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    const { stdout: showBefore } = await execAsync(`bd show ${taskId}`, { maxBuffer: 1024 * 1024 });
    const statusMatch = showBefore.match(/Status:\s*(\w+)/);
    const originalStatus = statusMatch ? statusMatch[1] : 'open';
    
    // Update to in_progress
    await execAsync(`bd update ${taskId} --status in_progress`, { maxBuffer: 1024 * 1024 });
    console.log(`   ✓ Updated ${taskId} to in_progress`);
    
    // Verify update
    const { stdout: showAfter } = await execAsync(`bd show ${taskId}`, { maxBuffer: 1024 * 1024 });
    ...
    if (statusAfter === 'in_progress') {
      ...
      await execAsync(`bd update ${taskId} --status ${originalStatus}`, { maxBuffer: 1024 * 1024 });
      console.log(`   ✓ Restored status to ${originalStatus}`);
      
      results.push({ function: 'update(id, options)', status: 'success', taskId });
    } else {
      console.log('   ✗ Update not verified');
      results.push({ function: 'update(id, options)', status: 'failed', reason: 'Update not verified' });
    }
  } catch (error) {
    console.log('   ✗ update() failed:', error.message);
    results.push({ function: 'update(id, options)', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    // Close task
    await execAsync(`bd close ${taskId} --reason="Test close"`, { maxBuffer: 1024 * 1024 });
    ...
    if (statusAfter === 'closed') {
      ...
      results.push({ function: 'close(id, reason)', status: 'success', taskId });
    } else {
      console.log('   ✗ Close not verified');
      results.push({ function: 'close(id, reason)', status: 'failed', reason: 'Close not verified' });
    }
  } catch (error) {
    console.log('   ✗ close() failed:', error.message);
    results.push({ function: 'close(id, reason)', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    results.push({ function: 'sync()', status: 'warning', error: error.message });
  }
...
async function testBeadsViewerClient() {
  ...
  try {
    ...
    results.push({ function: 'triage()', status: 'failed', error: error.message });
  }

  // Test 2: plan(options)
  console.log('\n2. Testing plan(options)...');
  try {
    const { stdout } = await execAsync('bv --robot-plan', { maxBuffer: 5 * 1024 * 1024 });
    const planData = JSON.parse(stdout);
    console.log('   ✓ bv --robot-plan command works');
    console.log(`   ✓ Has plan: ${!!planData.plan}`);
    results.push({ function: 'plan(options)', status: 'success', hasPlan: !!planData.plan });
  } catch (error) {
    console.log('   ✗ plan() failed:', error.message);
    results.push({ function: 'plan(options)', status: 'failed', error: error.message });
  }

  // Test 3: insights(options)
  console.log('\n3. Testing insights(options)...');
  try {
    const { stdout } = await execAsync('bv --robot-insights', { maxBuffer: 5 * 1024 * 1024 });
    ...
    results.push({ function: 'insights(options)', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    results.push({ function: 'alerts()', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    results.push({ function: 'history()', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    results.push({ function: 'labelHealth()', status: 'failed', error: error.message });
  }
...
async function testBeadsMiddleware() {
  ...
  try {
    ...
    if (output.systemPrompt && output.systemPrompt.length > 0) {
      ...
      const hasRecommendations = output.systemPrompt.includes('Recommended Tasks');
      ...
      results.push({ function: 'formatRecommendations()', status: 'failed', reason: 'No output' });
    }
  } catch (error) {
    console.log('   ✗ formatRecommendations() failed:', error.message);
    results.push({ function: 'formatRecommendations()', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    const hasInsights = insightsData.Cycles || insightsData.Bottlenecks || 
                       insightsData.Keystones || insightsData.Stats;
    ...
    results.push({ function: 'formatInsights()', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    results.push({ function: 'formatAlerts()', status: 'failed', error: error.message });
  }
...
async function testPluginHooks() {
  ...
  try {
    ...
    const hasBeadsTriage = beforeOutput.beadsTriage && Object.keys(beforeOutput.beadsTriage).length > 0;

    console.log(`   ✓ Hook executed`);
    console.log(`   ✓ Has system prompt: ${hasSystemPrompt}`);
    console.log(`   ✓ Has beads triage: ${hasBeadsTriage}`);
    console.log(`   ✓ System prompt length: ${beforeOutput.systemPrompt?.length || 0}`);
    ...
    results.push({ hook: 'agent.execute.before', status: 'failed', error: error.message });
  }
...
async function runComprehensiveTest() {
  ...
  const beadsClientSuccess = beadsClientResults.filter(r => r.status === 'success').length;
  const beadsViewerSuccess = beadsViewerResults.filter(r => r.status === 'success').length;
  const middlewareSuccess = middlewareResults.filter(r => r.status === 'success').length;
  const hooksSuccess = hooksResults.filter(r => r.status === 'success').length;

  const totalTests = beadsClientResults.length + beadsViewerResults.length + 
                     middlewareResults.length + hooksResults.length;
  const totalSuccess = beadsClientSuccess + beadsViewerSuccess + 
                      middlewareSuccess + hooksSuccess;
  ...
  beadsClientResults.forEach(r => {
    const icon = r.status === 'success' ? '✓' : r.status === 'skipped' ? '○' : r.status === 'warning' ? '⚠' : '✗';
