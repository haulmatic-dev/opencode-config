...
function validateConfigForManager(config) {
  const errors = [];
...
const reloadManager = new HotReloadManager(
  CONFIG_PATH,
  validateConfigForManager,
);

reloadManager.on('change', ({ current }) => {
  if (current) {
    const processedConfig = processLoadedConfig(current);
    cachedConfig = processedConfig;
  }
});

reloadManager.on('error', (err) => {
  console.warn(`[Config] Watcher error: ${err.message}`);
});

reloadManager.on('config-invalid', (err) => {
  console.warn(`[Config] Invalid config detected, keeping previous: ${err}`);
});

async function processLoadedConfig(fileConfig) {
  let config = { ...DEFAULTS };
...
function syncLoadConfig() {
  if (cachedConfig !== null && initialLoadComplete) {
    return cachedConfig;
  }
...
async function startWatching() {
  try {
    await reloadManager.loadConfig();
    reloadManager.startWatching();
  } catch (err) {
    console.warn(
      `[Config] Warning: Failed to start config watcher: ${err.message}`,
    );
  }
}
...
export async function loadConfig() {
  ...
  if (!initialLoadComplete) {
    return syncLoadConfig();
  }

  try {
    const fileConfig = await reloadManager.reload();
    cachedConfig = await processLoadedConfig(fileConfig);
    return cachedConfig;
  } catch (error) {
    configLoadError = error;
    console.warn(`[Config] Warning: Failed to reload config: ${error.message}`);
    return cachedConfig;
  }
}

export function getConfig() {
  const config = syncLoadConfig();
  return config;
}
...
export function on(event, callback) {
  reloadManager.on(event, callback);
}
...
export function getSource() {
  return reloadManager.getConfig();
}
