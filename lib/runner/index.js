...
import { formatAgentForPrompt, loadAgent } from './agent-loader-registry.mjs';
import { parseArguments } from './args.mjs';
import reservationManager from './file-reservation.js';
...
async function validateReservations(filePatterns, taskId, agentType) {
  const reservations = reservationManager.status();
  const conflicts = [];

  for (const pattern of filePatterns) {
    for (const res of reservations) {
      if (reservationManager.matchesPatterns([pattern], res.filePatterns[0])) {
        if (res.owner !== `${taskId}-${agentType}`) {
          conflicts.push({
            file: pattern,
            reservedBy: res.owner,
...
async function releaseTaskReservations(taskId, agentType) {
  try {
    const count = await reservationManager.releaseByOwner(
      `${taskId}-${agentType}`,
    );
    if (count > 0) {
      console.log(`[Runner] Released ${count} reservations for task ${taskId}`);
    }
  } catch (error) {
    console.warn(`[Runner] Failed to release reservations: ${error.message}`);
  }
}

async function runAgent(taskId, agentType, workflowType, options = {}) {
  const { requireReservations = true, filePatterns = [] } = options;
  ...
  try {
    ...
    if (requireReservations && filePatterns.length > 0) {
      console.log(
        `[Runner] Validating file reservations for: ${filePatterns.join(', ')}`,
      );
      const conflicts = await validateReservations(
        filePatterns,
        taskId,
        agentType,
      );
      ...
      if (conflicts.length > 0) {
        console.error(`[Runner] ✗ File reservation conflicts detected:`);
        for (const conflict of conflicts) {
          console.error(
            `  - ${conflict.file} reserved by ${conflict.reservedBy}`,
          );
        }
        return {
          exitCode: 1,
          error: new Error('File reservation conflicts detected'),
          conflicts,
        };
      }

      console.log(
        `[Runner] Acquiring reservations for: ${filePatterns.join(', ')}`,
      );
      const reservation = await reservationManager.acquire(filePatterns, {
        owner: `${taskId}-${agentType}`,
        ttl: 120000,
      });
      reservationId = reservation.id;
      console.log(`[Runner] ✓ Reservations acquired: ${reservationId}`);
    }

    const agent = await loadAgent(agentType);
    if (agent) {
      console.log(`[Runner] Loaded agent: ${agent.id}`);
      console.log(`[Runner] Agent aliases:`, agent.aliases);
    } else {
      console.warn(`[Runner] ⚠ Agent file not found for: ${agentType}`);
    }
    ...
    const agentPrompt = formatAgentForPrompt(agent);
    ...
    if (reservationId) {
      await reservationManager.releaseById(reservationId);
      console.log(`[Runner] ✓ Reservations released: ${reservationId}`);
    }
  }
}
...
async function main() {
  ...
  if (args.workflow) {
    ...
    const result = await runAgent(args.task, args.agent, args.workflow, {
      requireReservations: args.requireReservations !== false,
