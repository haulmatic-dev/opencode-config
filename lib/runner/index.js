...
import { formatAgentForPrompt, loadAgentByName } from './agent-loader.js';
...
async function runAgent(taskId, agentType, workflowType) {
  ...
  try {
    ...
    const agent = await loadAgentByName(agentType);
    if (agent) {
      console.log(`[Runner] Loaded agent: ${agent.name}`);
      console.log(`[Runner] Agent metadata:`, agent.metadata);
    } else {
