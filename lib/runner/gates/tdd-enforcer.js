...
import { TestFileDetector } from '../utils/test-detector.js';

let testDetector = null;

async function getTestDetector() {
  if (!testDetector) {
    testDetector = new TestFileDetector();
    await testDetector.detectFramework();
  }
  return testDetector;
}
...
export async function verifyTDD(filesModified) {
  ...
  const detector = await getTestDetector();
  const testFiles = await detector.detectTestFiles(filesModified);
  const codeFiles = await detector.detectCodeFiles(filesModified);
...
export async function enforceRedGreenCycle() {
  ...
  const diffResult = await execAsync('git diff --name-only 2>&1');
  const changedFiles = diffResult.stdout.trim().split('\n').filter(Boolean);

  const detector = await getTestDetector();
  const testFiles = await detector.detectTestFiles(changedFiles);
  const codeFiles = await detector.detectCodeFiles(changedFiles);
  ...
  for (const commit of commits) {
    ...
    const hasTestFile = await detector.detectTestFiles(files);
    const hasCodeFile = (await detector.detectCodeFiles(files)).length > 0;
