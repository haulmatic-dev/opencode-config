...
export async function verifyMutation(filesModified) {
  ...
  try {
    ...
    console.error('[Gate: Mutation] Error:', error.message);
    return {
      passed: false,
      reason: `Mutation gate error: ${error.message}`,
...
export async function verifyTDD(filesModified) {
  ...
  try {
    console.log('[Gate: TDD] Running tests against current code...');
    const testResult = await execAsync('npm test 2>&1');
    const currentTestsPass = testResult.exitCode === 0;

    if (currentTestsPass) {
      return {
        passed: false,
        reason:
          'TDD violation: Tests pass without implementation. In strict TDD, new tests must fail on old code.',
        details: {
          currentCodeTestsPass: true,
    ...
    console.log(
      '[Gate: TDD] ✓ TDD verification passed - tests fail on old code',
    );
    return {
      passed: true,
      reason: 'Tests fail on old code (TDD compliant)',
      details: {
        currentCodeTestsPass: false,
      },
    };
  } catch (error) {
    console.error('[Gate: TDD] Error:', error.message);
    return {
      passed: false,
      reason: `TDD gate error: ${error.message}`,
      details: { error: error.message },
    };
  }
}
