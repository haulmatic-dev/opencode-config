...
async function testCassMemory() {
  ...
  try {
    ...
    const { stdout: contextOutput } = await execAsync(
      'cm context "test task" --json --limit 1',
    );
    const contextData = JSON.parse(contextOutput);
    console.log('   ✓ cm context works, success:', contextData.success);
    console.log(
      '   - relevantBullets:',
      contextData.data?.relevantBullets?.length || 0,
    );
    console.log(
      '   - antiPatterns:',
      contextData.data?.antiPatterns?.length || 0,
    );
    console.log(
      '   - historySnippets:',
      contextData.data?.historySnippets?.length || 0,
    );
    ...
    console.log(
      '- Agent Hooks: agent.execute.before (inject context), agent.execute.after (record outcome)',
    );
